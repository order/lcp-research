...
if __name__ == '__main__':
    tN = 21
    vN = 21
    discretizer = generate_discretizer((-3.0*np.pi / 2.0, np.pi / 2.0,tN),\
        (-np.pi,np.pi,tN),\
        (-10,10,vN),\
        (-10,10,vN),\
        (-10,10,5))
    ...
    if simulate_only:
        ...
        init_state = np.array([[-np.pi / 2.0 + 0.1,0.0,0.0,0.0]])
