...
void build_lcp(const po::variables_map & var_map,
  ...
  assert(NUM_ACTION+1 == blk_M.at(0).size());
  assert(NUM_ACTION+1 == blk_M.size());
  sp_mat M = bmat(blk_M);
  assert(M.is_square());
  assert((NUM_ACTION+1)*(N+1) == M.n_rows);

  // Get costs for spatial nodes, and add oob node
  double tail_factor = 1.0 / (1.0 - gamma);
  mat costs = build_car_costs(points,
                              NUM_ACTION, tail_factor);
  costs = join_vert(costs,tail_factor * ones<mat>(1,NUM_ACTION));
