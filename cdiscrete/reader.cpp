...
void read_mcts_config_file(Demarshaller & demarsh,
			   ...
			   RegGrid & ref_grid,
			   vec & ref_v){
  uint problem_type = (uint)demarsh.get_scalar("Problem type");
  switch(problem_type){
  case PROBLEM_DI:
    read_di_config_file(demarsh,
			grid,
			problem,
			context,
			sim_horizon,
			...
			ref_v);
    break;
  case PROBLEM_HILLCAR:
    read_hillcar_config_file(demarsh,
			     grid,
			     problem,
			     context,
			     sim_horizon,
...
void read_hillcar_config_file(Demarshaller & demarsh,
  ...
  // Assemble problem
  HillcarTransferFunction * hillcar_fn = new HillcarTransferFunction(step_len,
								     n_steps,
								     damp,
								     jitter);
