...
def test_centroid():
    ...
    S = np.random.multivariate_normal(np.zeros(D),
                                      np.eye(D), N)
    assert((N,D) == S.shape)
    cuts = partition_samples(S,K)
    part = IrregularGridPartition(cuts)
    disc = IrregularGridDiscretizer(part)
    indexer = Indexer(part.lens)
    cent = weighted_centroid(S,disc,indexer)

    coords = disc.to_cell_coord(S)
    indices = indexer.coords_to_indices(coords)

    for n in xrange(N):
        idx = indices[n]
        plt.plot([S[n,0],cent[idx,0]],
                 [S[n,1],cent[idx,1]],
                 '-b',alpha=0.2)
    plt.plot(cent[:,0],cent[:,1],'rs')
    plt.show()
