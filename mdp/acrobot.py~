...
class AcrobotRemapper(state_remapper.StateRemapper):
    ...
    def get_acc(self,q,dq,u):
        ...
        g_2 = m2*lc2*g*c12
        g_1 = (m1*lc1 + m2*l1)*g*c1 + g_2
        
        d = self.dampening
        ddq_2 = (h_11 * (u - c_2 - g_2 - d*dq[1]) + h_12*(c_1 + g_1 + d*dq[0])) / (h_11 * h_22 - h_12*h_21)
        ddq_1 = -(h_12*ddq_2 + c_1 + g_1 + d*dq[0]) / h_11
    ...
    def forward_kinematics(self,state):
        ...
        x1 = self.l1 * np.array([np.cos(q[0]),np.sin(q[0])])
        theta_12 = np.sum(q)
        
        # Position of end joint
        x2 = x1 + self.l2 * np.array([np.cos(theta_12),np.sin(theta_12)])
