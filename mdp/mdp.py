    ...
    def get_decisions(self,states):
        ...
        for (i,a) in enumerate(self.actions):
            vals[:,i] = self.cost_obj.cost(states,a) \
                + self.discount*self.value_fun_eval.evaluate(next_states[:,:,i])
...
class KStepLookaheadPolicy(Policy):
    ...
    def __get_vals(self,states,k):
        ...
        if k <= 0:
            ...
            vals = np.tile(self.value_fun_eval.evaluate(states),(1,1))
            return vals
    ...
    def get_decisions(self,states):
        ...
        centered = vals - np.tile(np.mean(vals,axis=1)[np.newaxis,:].T,(1,vals.shape[1]))
        plt.imshow(centered)
        plt.show()
        quit()
