...
|/  2|
c----d
Triangle 1 and 2 share edge cd.
Note that triangle acb is counter-clockwise (CCW),
 and it's mirrored version dcb is CW.
All tets must be in CCW orientation.
...
refering to mirror tets for every face.
...
the stream operator (>>)
NB: The tets MUST be in CCW order---see the reorienting function.
In particular, any non-finite tet's (tets that contain the 0 vertex id)
