...
Modified by Erik Zawadzki (epz@cs.cmu.edu) on 2016-09-12
Added support for INRIA Medit files and some kind of format CGAL uses
for exporting triangulations.
"""
   ...
   meshconvert.py [-s scale] input_file output_file
       ...
       input_file    the input tetrahedral (or trianglar surface) mesh file. 
                     This file must be in one of the following formats:
                         ...
                         .medit - CGAL's MEDIT file format
...
def ReadINRIAMedit(fileName):
    fileName += '.medit'
    FH = open(fileName)
    lines = FH.readlines()
    FH.close()
    lines = [x.split('#')[0].strip() for x in lines] # Strip comments
    ...
    for name in names:
        ...
        exec(name + ' = objs')
        I += n
    triangles = triangles.astype(np.integer) - 1 # 0 is the node at infinity in MEDITs
    tetrahedra = tetrahedra.astype(np.integer) - 1
    

    for tetNum, tet in enumerate(tetrahedra):
        a = vertices[int(tet[0])]
        b = vertices[int(tet[1])]
        c = vertices[int(tet[2])]
        d = vertices[int(tet[3])]
        # if tet is negative orientation, flip two verts
        if orient3d(a,b,c,d) == 0:
            print "WHOA! input zero-volume tet...\n"
        if orient3d(a,b,c,d) < 0:
            tetrahedra[tetNum][-1],tetrahedra[tetNum][-2] = tet[-2],tet[-1]
            
    if True:
        faces, bound_faces, face2tet = GetFaceTopo(tetrahedra)
    else:
        bound_faces = None
        
    return vertices, tetrahedra, bound_faces
...
def writeEle(tets, outFileName):
    ...
    for iter in range(0,len(tets)):
        ...
        outfile.write(str(iter+1) + ' ' + ' '.join(map(str,map(int,tets[iter]))) + '\n')
    pass
